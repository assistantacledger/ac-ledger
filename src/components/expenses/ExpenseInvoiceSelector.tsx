...
export function ExpenseInvoiceSelector({ employeeName, expenses, onGenerate, onClose }: Props) {
  const [selected, setSelected] = useState<Set<string>>(
    () => new Set(
      expenses
        .filter(e => (e.status === 'submitted' || e.status === 'approved') && !e.invoiced)
        .map(e => e.id)
    )
  )
  ...
  // Unique project options for the dropdown
  const projectOptions = useMemo(() => {
    const seen = new Set<string>()
    const opts: { value: string; label: string }[] = []
    for (const exp of expenses) {
      const value = exp.project_code || '—'
      if (!seen.has(value)) {
        seen.add(value)
        opts.push({
          value,
          label: exp.project_code
            ? `${exp.project_code}${exp.project_name ? ' — ' + exp.project_name : ''}`
            : '— No Project',
        })
      }
    }
  ...
  return (
    <>
      ...
      >
        <div
          className="bg-white max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl"
        ...
        >
          ...
          {/* Filters */}
          <div className="px-6 py-3 border-b border-rule bg-cream flex-shrink-0 flex flex-wrap items-center gap-2">
            {/* All / Unpaid toggle */}
            ...
            {projectOptions.length > 1 && (
              <select
                ...
                className="border border-rule bg-white px-2 py-1.5 font-mono text-[10px] text-ink focus:outline-none focus:border-ink"
              >
                ...
                return (
                  <div key={group.label} className="border-b border-rule last:border-0">
                    ...
                    {!isCollapsed && group.exps.map(exp => (
                      <div key={exp.id} className="border-b border-rule/30 last:border-0">
                        <label className="flex items-start gap-3 px-6 py-2.5 hover:bg-cream/40 cursor-pointer transition-colors">
                          <input
                            type="checkbox"
                          ...
                          />
                          <span className="font-mono text-xs text-muted w-20 flex-shrink-0">
                            {fmtDate(exp.date)}
                          </span>
                          <span className="flex-1 text-xs text-ink leading-relaxed min-w-0">
                            {exp.notes || '—'}
                          </span>

                          {/* Receipt thumbnails — click to preview, stop checkbox toggle */}
                              ...
                              {(exp.receipt_urls ?? []).map((url, i) => {
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={e => {
                                      e.stopPropagation()
                                      setPreview({ url, name: `Receipt ${i + 1} — ${exp.employee}` })
                                    }}
                                    title={`Preview receipt ${i + 1}`}
                                    className="w-6 h-6 border border-rule bg-white hover:border-ink transition-colors flex items-center justify-center overflow-hidden flex-shrink-0"
                                  >
                                    {isPdf ? (
                          ...
                          <span className="font-mono text-xs font-semibold text-ink w-20 text-right flex-shrink-0">
                            {fmt(exp.total)}
                          </span>
                          <span className={cn('badge flex-shrink-0', STATUS_BADGE[exp.status])}>
                            {exp.status}
                          </span>
                          {exp.invoiced && (
                            <span className="font-mono text-[9px] uppercase tracking-wider text-muted border border-muted/40 px-1.5 py-0.5 flex-shrink-0">
                              invoiced
                            </span>
                          )}
                        </label>
      ...
      {/* Receipt preview — rendered above the modal (z-[60] > z-50) */}
      {preview && (
        <FilePreviewOverlay
          url={preview.url}
          name={preview.name}
